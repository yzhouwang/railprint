...
test('zoom-LOD: low zoom shows only top tiers + ridden; zooming in reveals urban lines', async ({
  ...
  // The CI sandbox has no internet, and MapLibre won't fire 'load' until the basemap source
  // gets at least one tile. Serve a blank 1×1 PNG for the OSM raster so the map loads exactly
  // as in production — the LOD assertions only touch the local rail (GeoJSON) layers, never the
  // basemap, so a transparent tile is faithful.
