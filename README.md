...
> Working name. Status: **v0.13.4.0** — the real MLIT N02 national network (594 lines, ~10,000 stations,
...
> vehicle dex over a curated, fact-checked registry of 166 形式 in regular
> scheduled service, mid-2026); and the first
...
- **Mark a ride:** pick a line → tap station A → tap station B, **or** search two stations by 日本語 / romaji / かな and pick from the routes the app finds between them — even across multiple lines (a 特急ひのとり 津→大阪難波 through-service records as one trip). A searched route reads back for confirmation (経路を確認) — with 車両 suggestions scoped to that route's lines — and records only on an explicit 記録 tap. Live km + % (national, HSR) update.
- **Your rides are a journey log, not just coverage.** Marking a trip you've ridden before records a *new* dated journey (coverage % never double-counts). The **旅の記録 diary** lists each journey — date · route · km · line(s) · train — and an optional **車両** field collects the rolling stock you've ridden (N700S, CR400AF, …), canonicalized so variants fold into one model. Suggestion chips know each line's real service (per-line profiles, fact-checked against mid-2026 timetables): marking 東海道新幹線 offers N700A/N700S — never CR200J — a searched multi-line route leads with the stock serving every line of the route (長野→金沢 puts E7系/W7系 first), and a line without curated candidates says so instead of guessing; your own history and free-text entry are never filtered. Tap a diary pill to fix or add a trip's 車両 after the fact; a **未記録のみ** filter makes enriching an imported history visible progress.
- **車両図鑑 — a data-honest vehicle dex.** Every model you log becomes a collected card: per-model 先頭 (lead-car) artwork in true livery for all 13 active 新幹線 and 24 flagship 特急 (あずさ, サンダーバード, ソニック, ひのとり, ラピート…), claymation category silhouettes in livery tints for the rest, completion meters over curated **closed** rosters only (新幹線 X/13; 京沪で会える車両 X/4 — the CN meter counts what the corridor can actually deliver), a ghost want-list spanning the active Japan roster (every JR 特急 and the private-railway bench included) plus the four 京沪 corridor cards, with retirement urgency straight from roster facts (500系「2027年1月引退予定」), model detail pages (operator · top speed · 愛称 · your trips on it), and five milestone stamps. No gacha, no invented rarity; unknown free-text models stay first-class. Mark a new-to-you model and the toast becomes 「◯◯を図鑑に追加しました · 新幹線 8/13」 — tap through to the card.
...
beyond the 新幹線+特急 batches, batch backfill stepper, GPS check-in), community heatmap,
broad China beyond the 京沪 corridor, any backend.
